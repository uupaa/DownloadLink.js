...
DownloadLink["prototype"] = Object.create(DownloadLink, {
    ...
    "create": { "value": DownloadLink_create }, // #create(blob:Blob, options:Object = null):Boolean
});

// --- implements ------------------------------------------
function DownloadLink_create(blob,      // @arg Blob
                             options) { // @arg Object - { fileName, linkText, linkStyle, parentNode }
                                        // @options.fileName String = "unknown"
                                        // @options.linkText String = "download"
                                        // @options.linkStyle String = "DownloadLink" - CSS className
                                        // @options.parentNode Node = document.body
//{@dev
    if (VERIFY) {
        $valid($type(blob, "Blob"), DownloadLink_create, "blob");
